...
def compute_bending_energy(input_coords, output, batch_size=None, equalspacing=False):
    ...
    # Wolterink et al used yzfac=1 here, which was presumably a bug/typo. Kept as an option 
    # for comparison's sake. It has a very small impact on the results, (in-)stability 
...
def compute_balanced_jacobian_loss(input_coords, output, batch_size=None):
    """Compute the jacobian regularization loss."""
...
def compute_hessian_matrix(input_coords, output, add_identity=True):
    """Compute the Hessian wrt the input"""
    ...
    hessian_matrix = torch.zeros(input_coords.shape[0], 3, 3, device='cuda')
    hessian_matrix[:, :, ]
